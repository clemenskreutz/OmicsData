...
function PlotImputationSingle(O)
...
%            ylim([-0.02 5.05])
    ...
    set(gca,'XTickLabel',methodsorted,'XTickLabelRotation',45, 'FontSize',14);  
    ylabel('|Imputed - Original|', 'FontSize',14)
    legend(p2,'RMSE','Location','northwest')
    title('Imputation error', 'FontSize',18)
    %set(gca,'FontSize', 20)
    print([filepath '/' name '/' name '_Boxplot_Difference_' num2str(b)],'-dpng','-r50');


     %% Correlation of imp/orig
     figure; set(gcf,'units','normalized','outerposition',[0 0 .5 .8])                       
     methodhist = methodsorted(:,~all(isnan(X))); % if a method did not work
     X = X(:,~all(isnan(X)));
...
%                             end
    ...
    print([filepath '/' name '/' name '_Cloud6_' num2str(b)],'-dpng','-r50');

    % Table
    uitable('Data',T,'ColumnName',['original' method],'Units','Normalized','Position',[0, 0, 1, 1]);
    print([filepath '/' name '/' name '_Table_' num2str(b)],'-dpng','-r50');
end
end
...
print([filepath '/' name '/' name '_Histogram'],'-dpng','-r50');
